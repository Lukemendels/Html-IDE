...
(async () => {
  ...
  const unfencedLiteralBackticks = ['SEARCH:', '<pre id="output"></pre>', 'REPLACE:', '<pre id="output">```json', '{"status":"ok"}', '```</pre>'].join('\n');
  assert.equal(engine.parseLegacySearchReplace(unfencedLiteralBackticks).patches[0].replacement, '<pre id="output">```json\n{"status":"ok"}\n```</pre>', 'unfenced legacy replacement retains literal backticks');
  const commentaryBeforeLegacy = ['Here is the patch:', '```', 'SEARCH:', 'A', 'REPLACE:', 'B', '```'].join('\n');
  const commentaryAfterLegacy = ['```', 'SEARCH:', 'A', 'REPLACE:', 'B', '```', 'Apply this.'].join('\n');
  const twoFencedLegacy = ['```', 'SEARCH:', 'A', 'REPLACE:', 'B', '```', '', '```', 'SEARCH:', 'C', 'REPLACE:', 'D', '```'].join('\n');
  const twoFencedJson = ['```json', await packet(source, [p('first', 'replace', '<h1>Title</h1>', '<h1>One</h1>')]), '```', '```json', await packet(source, [p('second', 'replace', '<h1>Title</h1>', '<h1>Two</h1>')]), '```'].join('\n');
  const unclosedFence = ['```json', await packet(source, [p('unclosed', 'replace', '<h1>Title</h1>', '<h1>Nope</h1>')])].join('\n');
  const orphanClosingFence = ['SEARCH:', 'A', 'REPLACE:', 'B', '```'].join('\n');
  const fenceError = /fence|exactly one|commentary|multiple/i;
  assert.throws(() => engine.parsePatchPacket('Here is the patch:\n' + fencedJson), fenceError, 'commentary before fenced JSON rejected');
  assert.throws(() => engine.parsePatchPacket(fencedJson + '\nApply this.'), fenceError, 'commentary after fenced JSON rejected');
  assert.throws(() => engine.parsePatchPacket(commentaryBeforeLegacy), fenceError, 'commentary before fenced legacy rejected');
  assert.throws(() => engine.parseLegacySearchReplace(commentaryBeforeLegacy), fenceError, 'direct legacy rejects commentary before fence');
  assert.throws(() => engine.parsePatchPacket(commentaryAfterLegacy), fenceError, 'commentary after fenced legacy rejected');
  assert.throws(() => engine.parseLegacySearchReplace(commentaryAfterLegacy), fenceError, 'direct legacy rejects commentary after fence');
  assert.throws(() => engine.parsePatchPacket(twoFencedLegacy), fenceError, 'two fenced legacy packets rejected');
  assert.throws(() => engine.parsePatchPacket(twoFencedJson), fenceError, 'two fenced JSON packets rejected');
  assert.throws(() => engine.parsePatchPacket(unclosedFence), fenceError, 'unclosed fence rejected');
  assert.throws(() => engine.parsePatchPacket(orphanClosingFence), fenceError, 'orphan closing fence rejected');
  await reject('preflight rejects malformed fence envelope', engine.preflightPatchPacket(commentaryBeforeLegacy, source), fenceError);
